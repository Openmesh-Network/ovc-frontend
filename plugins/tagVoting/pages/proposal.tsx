...
import { useProposal } from "@/plugins/tagVoting/hooks/useProposal";
import { useProposalVoteList } from "@/plugins/tagVoting/hooks/useProposalVoteList";
import { ToggleGroup, Toggle } from "@aragon/ods";
import ProposalDescription from "@/plugins/tagVoting/components/proposal/description";
import VotesSection from "@/plugins/tagVoting/components/vote/votes-section";
import ProposalHeader from "@/plugins/tagVoting/components/proposal/header";
import { Address, Hash, formatUnits } from "viem";
import { useUserCanVote } from "@/plugins/tagVoting/hooks/useUserCanVote";
import VoteTally from "@/plugins/tagVoting/components/vote/tally";
import VotingModal from "@/plugins/tagVoting/components/vote/voting-modal";
import ProposalDetails from "@/plugins/tagVoting/components/proposal/details";
...
export default function ProposalDetail({
  ...
  } = useProposalVoting(plugin, proposalId);
  const {
    canExecute,
    executeProposal,
    isConfirming: isExecuteConfirming,
  } = useProposalExecute(plugin, proposalId);
  const votes = useProposalVoteList(plugin, proposalId, proposal);
  const userCanVote = useUserCanVote(plugin, proposalId);
